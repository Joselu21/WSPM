...
def Help():
    ...
    print('Usage: wspm -Pm [product_file] -U [user_config]')
...
def PriceMonitoring(product_file = None, user_config = None):
    if(product_file != None):
        ...
        filenames = []
        for filename in glob.iglob("data/product/" + '**/*.txt', recursive=True):
...
def main():
    ...
    if("-h" in opts):
        ...
        product_index = sys.argv.index("-Pm") + 1
        if(len(sys.argv) - product_index > 0):
